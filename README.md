...
## 💾 Резервное копирование и безопасность данных (SQLite WAL)

DevFlow использует SQLite в режиме **WAL (Write-Ahead Logging)** для максимальной скорости и устойчивости к сбоям.

### Как безопасно делать бэкапы:

#### Способ 1: Встроенный JSON-экспорт (Рекомендуется)
- **Через веб-интерфейс**: В профиле пользователя или в Настройках нажмите кнопку **«Скачать JSON»**.
- **Через REST API**:
  ```bash
  curl -H "Authorization: Bearer <ВАШ_ТОКЕН>" https://ВАШ_ДОМЕН/api/v1/vault/export > devflow_backup.json
  ```
  *Этот способ на 100% безопасен, изолирован на уровне транзакций и позволяет переносить записи между разными инстансами.*

#### Способ 2: Горячий онлайн-бэкап SQLite (Live Hot Backup)
Если вам нужен побайтовый снимок всей базы данных без остановки контейнера:
```bash
docker exec devflow sqlite3 /data/devflow.db "VACUUM INTO '/data/devflow_backup_$(date +%Y%m%d).db'"
```
Команда `VACUUM INTO` атомарно создает консистентную копию базы данных, автоматически объединяя WAL-журнал.

#### Способ 3: Бэкап Volume / Файлов
При плановой остановке контейнера (`docker stop devflow` или редеплое в Coolify) сервер выполняет **Graceful Shutdown**: автоматически запускает `PRAGMA wal_checkpoint(TRUNCATE)`, сбрасывая все транзакции из `-wal` файла прямо в `devflow.db`. После остановки можно безопасно копировать всю папку `/data`.
